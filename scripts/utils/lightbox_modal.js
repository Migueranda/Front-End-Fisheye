...
function displayMediaLightBox(index){
    ...
    let multimedia = '';
     ...
     if (global_media[index].video == undefined){// condition pour affiche une image ou une video
         multimedia = `<img src="assets/images/${global_media[index].image}" id="lightbox_select" class="media " />`                
     ...
     if (index > 0) { 
         index_prec = index - 1
         chevron_prec_html = `<span class="chevron-left" id="precedent" onclick="changeSlide(${index_prec})"><i class="fa-solid fa-chevron-left"></i></span>`
     }
     // chevron suivant*
     let index_suivant = index + 1
     let chevron_suivant_html = `<span class="chevron-right" id="suivant" onclick="changeSlide(${index_suivant})"><i class="fa-solid fa-chevron-right"></i></span>`
     ...
     <h3 class="titre_media">${global_media[index].title}</h3>`
